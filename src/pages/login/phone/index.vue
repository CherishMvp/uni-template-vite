<template>
  <view class="bg-white h-full overflow-hidden">
    <view class="mt-8">
      <image src="@/assets/images/logo.png" alt="" class="w-20 h-20 block mx-auto" />
      <view class="text-2xl text-center mt-4 font-bold">
        {{ title }}
      </view>
    </view>
    <view class="px-4 mt-18">
      <u-button type="success" shape="circle" ripple @click="handleLogin">
        <!-- open-type="getPhoneNumber" -->
        <!-- @getphonenumber="handleLogin" -->
        手机号快捷登录
      </u-button>
      <view class="text-center mt-3">
        <u-checkbox v-model="agree" class="">
          我已阅读并同意
          <span class="text-primary active:text-primary-700" @click.stop="handleAgree">《产品服务协议》</span>
        </u-checkbox>
      </view>
    </view>
