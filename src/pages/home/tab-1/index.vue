<template>
  <view class="h-full">
    <view class="flex items-center bg-gradient-to-l to-primary from-green-700 px-4 h-[30vh]" @click="handleLogin">
      <view class="overflow-hidden rounded-full w-20 h-20">
        <!-- :src="userData.headImage" -->
        <image src="@/assets/images/image-user.png" alt="" class="w-full h-full" />
      </view>
    ...
    <view class="">
      <view v-for="(item, index) of listModel" :key="index" class="flex items-center py-4 px-4 border-b border-gray-200 bg-white" @click="handleInfo(item)">
    ...
    <view v-if="isLogin" class="mt-8 px-4">
      <u-button type="success" plain ripple @click="handleLogout"> 退出登录 </u-button>
    </view>
  </view>
</template>
